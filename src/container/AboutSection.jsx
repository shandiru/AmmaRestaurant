...
export default function AboutScrollSection() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // WHO WE ARE (move right)
      if (img1Ref.current) {
        const move1 = Math.min(Math.max(progress * 50, 0), 50);
        img1Ref.current.style.transform = `translateX(${move1}%)`;
      }

      // WHAT WE DO (move left)
      if (img2Ref.current) {
        const move2 = Math.min(Math.max((progress - 1) * 50, 0), 50);
        img2Ref.current.style.transform = `translateX(${-move2}%)`;
      }
    };
